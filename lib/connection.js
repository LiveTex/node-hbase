...
Connection.prototype.makeRequest = function(method, command, data, callback,
  ...
  var req = http.request({
    ...
    res.on("close", function() {
      callback(Error("Connection closed"), null);
    });
  ...
  function localComplete() {
    if (timeoutItem !== null) {
      exec.cron.clearTimeout(timeoutItem);
    }
  }

  if (typeof opt_timeout === 'number' && opt_timeout !== 0) {
    timeoutItem = exec.cron.setTimeout(function() {
      req.abort();
      localComplete();
    }, opt_timeout);
  }
 ...
 * @return {number}
 */
Connection.prototype.get = function(command, callback, opt_timeout) {
  return this.makeRequest("GET", command, "", callback, opt_timeout);
};
 ...
 * @return {number}
 */
Connection.prototype.put = function(command, data, callback, opt_timeout) {
  return this.makeRequest("PUT", command, data, callback, opt_timeout);
};
 ...
 * @return {number}
 */
Connection.prototype.post = function(command, data, callback, opt_timeout) {
  return this.makeRequest("POST", command, data, callback, opt_timeout);
};
 ...
 * @return {number}
 */
Connection.prototype["delete"] = function(command, callback, opt_timeout) {
  return this.makeRequest("DELETE", command, "", callback, opt_timeout);
};
...
Connection.prototype.handleJson = function(response, body) {
  ...
  switch (response.statusCode) {
    ...
    default:
      e = new Error(response.statusCode + ": " +
          this.codes[response.statusCode]);
